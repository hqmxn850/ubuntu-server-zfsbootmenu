...
##Variables:
ubuntuver="questing" #Ubuntu release to install. "jammy" (22.04). "noble" (24.04). "questing" (25.10).
...
user="masashi" #Username for new install.
PASSWORD="alicia" #Password for user in new install.
hostname="manila" #Name to identify the main system on the network. An underscore is DNS non-compliant.
...
locale="ja_JP.UTF-8" #New install language setting.
timezone="Asia/Tokyo" #New install timezone setting.
...
swap_size="65536" #Swap partition size in mebibytes (MiB). Size of swap will be larger than defined here with Raidz topologies.
datapool="zpool" #Non-root drive data pool name.
...
remoteaccess_first_boot="yes" #"yes" to enable remoteaccess during first boot. Recommend leaving as "no" and run script with "remoteaccess". See notes in section above.
